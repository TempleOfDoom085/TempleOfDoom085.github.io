...
function carMesh(color = 0xff3333) {
  ...
  const body  = new THREE.MeshStandardMaterial({color, roughness:0.22, metalness:0.12});
  const glass = new THREE.MeshStandardMaterial({color:0xaaccee, transparent:true, opacity:0.55, roughness:0.04, metalness:0.0});
  const tyre  = new THREE.MeshStandardMaterial({color:0x111111, roughness:0.80, metalness:0.0});
  const rim   = new THREE.MeshStandardMaterial({color:0xcccccc, roughness:0.18, metalness:0.88});
  const hl    = new THREE.MeshStandardMaterial({color:0xffffcc, emissive:0xffff00, roughness:0.12, metalness:0.0});
  const tail  = new THREE.MeshStandardMaterial({color:0xff4444, emissive:0x880000, roughness:0.15, metalness:0.0});
  ...
  [[-1.7,-2.2],[-1.7,1.9],[1.7,-2.2],[1.7,1.9]].forEach(([wx,wz])=>{
    add(new THREE.CylinderGeometry(0.56,0.56,0.38,14), tyre, wx,0.28,wz, 0,0,Math.PI/2);
    add(new THREE.CylinderGeometry(0.34,0.34,0.44,10), rim,  wx,0.28,wz, 0,0,Math.PI/2);
  });
  [[-1.1,3.1],[1.1,3.1]].forEach(([hx,hz])=>add(new THREE.BoxGeometry(0.5,0.28,0.12),hl,hx,0.72,hz));
  [[-1.0,-3.2],[1.0,-3.2]].forEach(([tx,tz])=>add(new THREE.BoxGeometry(0.42,0.22,0.1),tail,tx,0.72,tz));
  return g;
}
...
function bikeMesh(color = 0x111122) {
  ...
  const body = new THREE.MeshStandardMaterial({color, roughness:0.18, metalness:0.14});
  const tyre = new THREE.MeshStandardMaterial({color:0x111111, roughness:0.80, metalness:0.0});
  const rim  = new THREE.MeshStandardMaterial({color:0xbbbbbb, roughness:0.20, metalness:0.85});
  const red  = new THREE.MeshStandardMaterial({color:0xcc2200, roughness:0.30, metalness:0.0});
  const bar  = new THREE.MeshStandardMaterial({color:0x888888, roughness:0.35, metalness:0.75});
  ...
  [-1.05,1.05].forEach(wz=>{
    add(new THREE.CylinderGeometry(0.48,0.48,0.22,14), tyre, 0,0.38,wz, 0,0,Math.PI/2);
    add(new THREE.CylinderGeometry(0.28,0.28,0.26,10), rim,  0,0.38,wz, 0,0,Math.PI/2);
  });
  add(new THREE.BoxGeometry(1.0,0.07,0.07), bar, 0,1.12,0.85);
  return g;
}

function horseMesh() {
  const g = new THREE.Group();
  const h = new THREE.MeshStandardMaterial({color:0x8B4513, roughness:0.90, metalness:0.0});
  const m = new THREE.MeshStandardMaterial({color:0x3a1f00, roughness:0.92, metalness:0.0});
  const add= (geo,mat,px,py,pz,rx=0)=>{const mesh=new THREE.Mesh(geo,mat);mesh.position.set(px,py,pz);mesh.rotation.x=rx;mesh.castShadow=true;g.add(mesh);return mesh;};
  ...
  [[-0.4,-0.9],[-0.4,0.8],[0.4,-0.9],[0.4,0.8]].forEach(([lx,lz],i)=>{
    ...
    leg.position.set(lx,0.5,lz); leg.name=`hl${i}`; leg.castShadow=true; g.add(leg);
  });
...
function planeMesh() {
  ...
  const wh  = new THREE.MeshStandardMaterial({color:0xdddddd, roughness:0.35, metalness:0.18});
  const red = new THREE.MeshStandardMaterial({color:0xdd2222, roughness:0.28, metalness:0.0});
  const gls = new THREE.MeshStandardMaterial({color:0x88aadd, transparent:true, opacity:0.55, roughness:0.04, metalness:0.0});
  ...
  update(dt, time, input, audio) {
    this.list.forEach(v => {
      ...
      if (v.type==='plane') {
        ...
        v.mesh.position.set(v.x,v.y,v.z);
        v.mesh.rotation.y=v.angle; v.mesh.rotation.x=-v.pitch;
